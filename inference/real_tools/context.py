...
class SessionContext:
    ...
    read_only_sync_policy: str | None = None
    ...
    def _normalize_tool_call_for_sync(tool_call: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        name = normalized_call.get("name") or normalized_call.get("function") or ""
        raw_arguments = normalized_call.get("arguments") or normalized_call.get("args") or {}
        ...
        return {
            ...
            "toolkit": normalized_call.get("toolkit"),
        }
    ...
    def flush_buffer(self) -> Optional[Dict[str, Any]]:
        ...
        normalized_calls = [
            normalized
            for normalized in (self._normalize_tool_call_for_sync(tc) for tc in self._buffer)
            if normalized is not None
        ]
        logger.info(
            "[CONTEXT] Flushing %d buffered real tool observations/updates",
            len(normalized_calls),
        )

        if not normalized_calls:
            self.clear_buffer()
            logger.info("[CONTEXT] No valid tool observations/updates to flush")
        ...
        if self.read_only_sync_policy:
            payload["read_only_sync_policy"] = self.read_only_sync_policy
