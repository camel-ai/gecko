...
def load_predefined_schema(
    ...
    Args:
        ...
        schema_dir: Directory to search for schema files.
            ...
            1) data/taubench/openapi/real
            2) data/taubench/openapi
            3) data/bfcl/openapi/multi_turn/real
    ...
    if schema_dir is None:
        ...
        schema_dirs = [
            repo_root / "data" / "taubench" / "openapi" / "real",
            repo_root / "data" / "taubench" / "openapi",
            repo_root / "data" / "bfcl" / "openapi" / "multi_turn" / "real",
        ]
    else:
...
def fix_openai_schema(schema: Dict[str, Any], depth: int = 0, path: str = "") -> Dict[str, Any]:
    ...
    Args:
        ...
        path: Current path in schema tree
...
class RealToolWrapper(FunctionTool):
        ...
        if openai_tool_schema is None:
            ...
            if openai_tool_schema is None:
                raise ValueError(
                    ...
                    "Add it to a *RealAPI.json file under data/taubench/openapi or "
                    "data/bfcl/openapi/multi_turn/real, or pass openai_tool_schema explicitly."
                )

        # Ensure schemas comply with OpenAI strict requirements
        openai_tool_schema = fix_openai_schema(openai_tool_schema)
        if isinstance(openai_tool_schema, dict):
            fn_schema = openai_tool_schema.get("function", {})
            raw_toolkit = (
                openai_tool_schema.get(X_BFCL_TOOLKIT_KEY)
                or (fn_schema.get(X_BFCL_TOOLKIT_KEY) if isinstance(fn_schema, dict) else None)
            )
            self.toolkit_name = canonicalize_bfcl_multi_turn_toolkit(raw_toolkit)
    ...
    def _sync_config_to_mock_server(
        ...
        logger.debug(f"[WRAPPER] Syncing {self.function_name} to {context.mock_server_url}, session={context.session_id}")
        ...
        if self.toolkit_name:
            tool_call["toolkit"] = self.toolkit_name
        tool_call = normalize_bfcl_multi_turn_tool_call(
            tool_call,
            rename=True,
        ...
        if context.buffer_updates:
            ...
            logger.debug(f"[WRAPPER] Buffered config update for {self.function_name}")
            return
