...
DEFAULT_MOCK_TOOL_TIMEOUT_SECONDS = 180.0


def _mock_tool_timeout_seconds() -> float:
    raw_value = os.getenv("GECKO_MOCK_TOOL_TIMEOUT_SECONDS", "").strip()
    if not raw_value:
        return DEFAULT_MOCK_TOOL_TIMEOUT_SECONDS
    try:
        timeout_seconds = float(raw_value)
    except ValueError:
        logger.warning(
            "Invalid GECKO_MOCK_TOOL_TIMEOUT_SECONDS=%r; using %.1fs",
            raw_value,
            DEFAULT_MOCK_TOOL_TIMEOUT_SECONDS,
        )
        return DEFAULT_MOCK_TOOL_TIMEOUT_SECONDS
    if timeout_seconds <= 0:
        logger.warning(
            "Non-positive GECKO_MOCK_TOOL_TIMEOUT_SECONDS=%r; using %.1fs",
            raw_value,
            DEFAULT_MOCK_TOOL_TIMEOUT_SECONDS,
        )
        return DEFAULT_MOCK_TOOL_TIMEOUT_SECONDS
...
def _generate_timed_openapi_funcs(
    toolkit: Any,
    api_name: str,
    openapi_spec: Dict[str, Any],
    timeout_seconds: float,
) -> List[Callable]:
    """Generate CAMEL-style OpenAPI callables with an explicit HTTP timeout."""
    from camel.toolkits.open_api_toolkit import openapi_security_config

    if getattr(toolkit, "_override_server_url", None):
        base_url = toolkit._override_server_url
    else:
    ...
    def _make_func(path: str, method: str, operation: Dict[str, Any], function_name: str):
        def openapi_function(**kwargs):
            request_url = f"{str(base_url).rstrip('/')}/{path.lstrip('/')}"
            headers: Dict[str, Any] = {}
            params: Dict[str, Any] = {}
            cookies: Dict[str, Any] = {}

            session_id = getattr(toolkit, "_session_id", None)
            if session_id:
                headers["X-Session-ID"] = session_id

            sec_requirements = operation.get("security", openapi_security)
            avail_sec_requirement: Dict[str, Any] = {}
            ...
            for sec_scheme_name, _ in avail_sec_requirement.items():
                try:
                    ...
                    raise ValueError(
                        f"`{api_key_name}` not found in environment variables. "
            ...
            for param in operation.get("parameters", []):
                ...
                if input_param_name not in kwargs:
                    continue
                if param["in"] == "path":
            ...
            if "requestBody" in operation:
                ...
                if not content_type_list:
                    raise ValueError("Missing request body content type")
            ...
            try:
                response = requests.request(method.upper(), request_url, **request_kwargs)
            except requests.Timeout as exc:
                raise TimeoutError(
                    f"Mock OpenAPI request timed out after {timeout_seconds:.1f}s: "
    ...
    for path, methods in openapi_spec.get("paths", {}).items():
        for method, operation in methods.items():
            ...
            functions.append(_make_func(path, method, operation, function_name))
...
class ToolRegistry:
    ...
    def register_mock_tools_from_openapi(
        ...
        Example:
            ...
            ...     "data/bfcl/openapi/gorilla_file_system.json",
        ...
        try:
            ...
            # Generate functions and schemas. CAMEL's generated OpenAPI
            # callables use requests without a timeout; Gecko mock responses
            # can legitimately take longer than 30s when LLM-backed response
            # generation is involved, but they should still be bounded.
            funcs = _generate_timed_openapi_funcs(
                toolkit,
                api_name,
                openapi_spec,
                timeout_seconds=_mock_tool_timeout_seconds(),
            )
    ...
    def bind_session_context(
        ...
        with self._session_lock:
            context = SessionContext(
                ...
                read_only_sync_policy=read_only_sync_policy,
            )
