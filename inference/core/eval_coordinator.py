...
_DEFAULT_GENERATED_BASE_CHECKLIST_ITEMS = [
...
class EvalCoordinator:
    ...
    def __init__(
        ...
        self.append_base_checklist_to_generated = append_base_checklist_to_generated

    def _base_checklist(self, *, include_default: bool = False) -> List[Dict[str, str]]:
        if self.base_checklist_items is None:
            items = _DEFAULT_GENERATED_BASE_CHECKLIST_ITEMS if include_default else []
        elif isinstance(self.base_checklist_items, list):
            items = self.base_checklist_items
        else:
            return []
        return [
    ...
    def _append_base_checklist_items(self, checklist: List[Dict]) -> List[Dict]:
        base_items = self._base_checklist(include_default=True)
        if not base_items:
            return list(checklist or [])
        return list(checklist or []) + base_items
        ...
        try:
            if self.enable_checklist:
                ...
                try:
                    ...
                    pass
                if self.append_base_checklist_to_generated:
                    checklist = self._append_base_checklist_items(checklist)
                    logger.info("Checklist with base items: %d items", len(checklist))
            else:
                fixed_checklist = self._base_checklist(include_default=False)
                if fixed_checklist:
                    checklist = fixed_checklist
                    logger.info(
                    ...
                    checklist = []
                    logger.info("Checklist disabled: no checklist prompt or fixed base items")
    ...
    def evaluate_attempt(
        ...
        try:
            ...
            judge_agent_response = strip_reasoning(agent_response or "") if self.include_agent_response_in_judge else None
            judgment_results, critical, score = self.evaluator.judge_execution(
                ...
                policy_cache_scope=policy_cache_scope,
            )
            ...
            pass_item = {
                "name": "judge_evaluation_unavailable",
                "description": "Judge evaluation failed; preserving this attempt as passing.",
                "reasoning": f"Judge exception recorded: {str(e)}",
                "status": "completed",
            }
            feedback = {
                "judge_failure": True,
                "error": str(e),
                "judgment_results": [pass_item],
                "failed_items": [],
            }
            return 1.0, feedback, [pass_item]
