...
_VALIDATION_MODEL_DISABLED = "__VALIDATION_MODEL_DISABLED__"
...
def set_model_config(
    ...
    normalized_validation_model = _normalize_model_name(validation_model)
    if (
        normalized_validation_model is None
        and isinstance(validation_model, str)
        and validation_model.strip().lower() in {"none", "null", ""}
    ):
        _validation_model = _VALIDATION_MODEL_DISABLED
    else:
        _validation_model = normalized_validation_model or "gpt-4.1-mini"
...
def get_validation_model() -> Optional[str]:
    """Get the validation model name."""
    if _validation_model == _VALIDATION_MODEL_DISABLED:
        return None
