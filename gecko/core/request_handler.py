...
class RequestHandler:
    ...
    async def handle_request(self, request: Request, matching_path: str, operation: Dict[str, Any], api_name: str) -> JSONResponse:
        ...
        try:
            ...
            if not is_valid:
                logging.error("Request validation failed (request_id=%s): %s", request_id, error_message)
                return JSONResponse(
                    status_code=422,
            ...
            logging.error("Request handling error (request_id=%s): %s", request_id, str(e))
            return JSONResponse(
                status_code=500,
    ...
    async def _generate_response(self, request: Request, operation: Dict[str, Any]) -> JSONResponse:
        ...
        if 'content' in success_response:
            ...
            if 'application/json' in content:
                ...
                if not is_valid:
                    logging.error(
                        "Generated response validation failed (request_id=%s): %s",
